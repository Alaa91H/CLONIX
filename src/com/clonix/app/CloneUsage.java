...
public class CloneUsage {
    ...
    /** Storage queries hit system service + disk scan: cache 30s per clone. */
    private static final long USAGE_TTL_MS = 30000;

    private static final class UsageEntry {
        final Usage usage;
        final long at;
        UsageEntry(Usage u, long t) { usage = u; at = t; }
    }

    private static final java.util.concurrent.ConcurrentHashMap<String, UsageEntry> usageCache =
            new java.util.concurrent.ConcurrentHashMap<>();

    private static String key(String pkg, int userId) { return pkg + "#" + userId; }

    public static void invalidate(String pkg, int userId) {
        if (pkg != null) usageCache.remove(key(pkg, userId));
    }

    public static void invalidateAll() { usageCache.clear(); }
    ...
    public static Usage queryPackage(Context c, String pkg, int userId) {
        if (pkg != null) {
            UsageEntry hit = usageCache.get(key(pkg, userId));
            if (hit != null && System.currentTimeMillis() - hit.at < USAGE_TTL_MS) return hit.usage;
        }
        Usage u = queryPackageUncached(c, pkg, userId);
        if (pkg != null && u.available) {
            usageCache.put(key(pkg, userId), new UsageEntry(u, System.currentTimeMillis()));
        }
        return u;
    }

    /** Cache-only peek for instant first paint (null = not cached yet). */
    public static Usage peek(String pkg, int userId) {
    ...
    /**
     * Batch variant for lists: reuses the same 30s cache, so scrolling
     * StorageActivity or reopening it does zero system-service calls.
     * Must be called off the main thread (queries can block on disk I/O).
     */
    public static java.util.Map<String, Usage> queryBatch(Context c,
            java.util.List<CloneStore.Clone> clones) {
        java.util.Map<String, Usage> out = new java.util.HashMap<>();
        if (clones == null) return out;
        for (CloneStore.Clone cl : clones) {
            if (cl == null || cl.pkg == null) continue;
            out.put(key(cl.pkg, cl.userId), queryPackage(c, cl.pkg, cl.userId));
        }
        return out;
    }

    private static Usage queryPackageUncached(Context c, String pkg, int userId) {
        Usage u = new Usage();
        try {
